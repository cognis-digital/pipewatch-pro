...
**pipewatch-pro** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `pipewatch-pro` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
pipewatch-pro ... --format json | cognis-connect emit --to sigma   # Sigma rules
pipewatch-pro ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
pipewatch-pro ... --format json | cognis-connect emit --to elastic --url $URL --token $TOK   # Elastic _bulk
pipewatch-pro ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `pipewatch-pro` outputs:

```python
from cognis_connect import normalize, sigma
findings = [normalize(rec, source="pipewatch-pro") for rec in records]   # records = your JSON output
