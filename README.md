...
```mermaid
flowchart LR
  IN[target / manifest] --> P[pipewatch-pro<br/>checks + rules]
...
`pipewatch-pro` is interoperable with every popular way of using AI:

- **MCP server** — `pipewatch-pro mcp` (Claude Desktop, Cursor, Cognis.Studio, [uncensored-fleet](https://github.com/cognis-digital/uncensored-fleet))
- **OpenAI-compatible / JSON** — pipe `pipewatch-pro scan . --format json` into any agent or LLM
...
```bash
pip install "git+https://github.com/cognis-digital/pipewatch-pro.git"    # pip (works today)
...
pip install cognis-pipewatch-pro                                          # PyPI (when published)
docker run --rm ghcr.io/cognis-digital/pipewatch-pro:latest --help        # Docker
brew install cognis-digital/tap/pipewatch-pro                             # Homebrew tap
curl -fsSL https://raw.githubusercontent.com/cognis-digital/pipewatch-pro/main/install.sh | sh
```
