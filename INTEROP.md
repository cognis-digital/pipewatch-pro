...
How **pipewatch-pro** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `pipewatch-pro` lives in the **Cloud / supply-chain hardening** cluster.
...
| from | relation | to |
|---|---|---|
| `pipewatch-pro` | JSON findings compose with | [`sbomgen`](https://github.com/cognis-digital/sbomgen), [`cvecheck`](https://github.com/cognis-digital/cvecheck), [`k8sharden`](https://github.com/cognis-digital/k8sharden), [`tfscan`](https://github.com/cognis-digital/tfscan) |
| `pipewatch-pro` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `pipewatch-pro` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `pipewatch-pro` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the cloud / supply-chain hardening cluster.** `pipewatch-pro` output feeds the next tool:
```bash
pipewatch-pro ... --format json > out.json          # this tool's findings (see `pipewatch-pro --help`)
...
pipewatch-pro ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
pipewatch-pro ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
pipewatch-pro ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `pipewatch-pro`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `pipewatch-pro` slots
